...
local Settings = {
    surface = nil, ---@type LuaSurface
    bossSpiderStartingLeftDistance = 5000,
    bossSpiderStartingVerticalDistance = 256, -- Half the height of each teams map area.
    spiderDamageToRetreat = 500,
    spiderDamageSecondsConsidered = 30, -- How many seconds the spider will check back over to see if it has sustained enough damage to retreat.
...
Spider.OnStartup = function()
    ...
    Settings.surface = game.surfaces[1] -- TODO: this may need updating to the one we use with the map generation settings when merged in to the main code.
...
Spider.CreateSpider = function(playerTeamName, spidersYPos, spiderColor, biterTeamName)
    ...
    local bossEntity = Settings.surface.create_entity {name = "jd_plays-jd_spider_race-spidertron_boss", position = spiderPosition, force = biterTeam}
...
Spider.WasDamagedInLastSecond = function(spider, currentTick)
    ...
    if spider.state ~= BossSpider_State.retreating then
        ...
        if damageSufferedRecently >= Settings.spiderDamageToRetreat then
            game.print("spider should retreat")
        --TODO: trigger a retreat
        end
    end
end
...
Spider.CheckRoamingForSecond = function(spider, spidersCurrentPosition)
    ...
    if spider.movementTargetPosition == nil then
        ...
        -- Without any water being on the map every tile should be reachable for the spider. So no validation of target required.
        local targetPosition = {x = math.random(spider.roamingXMin, spider.roamingXMax), y = math.random(spider.roamingYMin, spider.roamingYMax)}
        Spider.MoveSpiderToPosition(spider, targetPosition)
        ...
        if spidersCurrentPosition.x == spider.spiderPositionLastSecond.x and spidersCurrentPosition.y == spider.spiderPositionLastSecond.y then
            -- The spider has stopped moving so its arrived or got as close as it can. This should detect if it ever got jammed in 1 spot and restart the cycle to find a new movement target and hopefully un-jam it.
...
Spider.MoveSpiderToPosition = function(spider, position)
    spider.bossEntity.autopilot_destination = position
end
...
Spider.UpdatePlanRenders = function(spider)
    ...
    if spider.state == BossSpider_State.roaming then
        if spider.movementTargetPosition ~= nil then
            table.insert(spider.spiderPlanRenderIds, rendering.draw_text {text = "roaming - moving", surface = Settings.surface, target = spider.bossEntity, color = Colors.white, scale_with_zoom = true})
            table.insert(spider.spiderPlanRenderIds, rendering.draw_line {color = Colors.white, width = 2, from = spider.bossEntity, to = spider.movementTargetPosition, surface = Settings.surface})
        else
            table.insert(spider.spiderPlanRenderIds, rendering.draw_text {text = "roaming - arrived", surface = Settings.surface, target = spider.bossEntity, color = Colors.white, scale_with_zoom = true})
        end
    end
end

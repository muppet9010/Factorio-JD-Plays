...
---@class Halloween2023_LightExplosionDefinitions
...
---@field size double
---@field color? Color
---@field shift MapPosition

---@type Halloween2023_LightExplosionDefinitions[]
...
local BlankAnimations = {} ---@type table<uint, data.Animation>
for _, lightExplosionDefinition in pairs(LightExplosionDefinitions) do
    -- Get the right multipliers for this type.
    local time_Multiplier, intensity_Multiplier, size_Multiplier ---@type int, int, int
...
---@param animation data.Animation|data.AnimationVariations
...
---@param animationVariations data.AnimationVariations[]
...
for _, prototype in pairs(data.raw["fire"]) do
    ...
    if prototype.damage_per_tick ~= nil and prototype.damage_per_tick.type ~= nil and prototype.damage_per_tick.type == "fire" then
        ...
        if prototype.small_tree_fire_pictures ~= nil then
            ...
            ---@diagnostic disable: missing-fields # Temporary work around until Factorio docs fix bug: https://forums.factorio.com/viewtopic.php?f=233&t=109417
            prototype.light = {
...
local function RecordProjectileNameToAmmoCategory(action, ammoCategory)
    for _, triggerItem in pairs(action) do
        ...
        for _, triggerDelivery in pairs(triggerItem.action_delivery) do
            if triggerDelivery.type == "projectile" then
                ---@cast triggerDelivery data.ProjectileTriggerDelivery
